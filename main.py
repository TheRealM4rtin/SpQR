...
def get_inps(model, data_iterable, args, dev, nsamples=None):
    ...
    # Move embeddings to device and track original device
    emb = model.get_input_embeddings()
    emb_dev = emb.weight.device
    emb = emb.to(dev)
    
    # Move first layer to the same device as embeddings
    ...
    # Setup forward arguments
    forward_arg_names = ["attention_mask"]
    cache = {"i": 0, "attention_mask": None}

    # Catcher module remains the same
    ...
    # Restore original module and devices
